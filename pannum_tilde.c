...
t_int *pannum_tilde_perform(t_int *w)
{
    ...
    // Here I have utilized the current values output from the
    // [scaleroute] Brian has put up on git.  As it stands, 0 should
    // give a full amplitude centered stereo location. A value of 1
    // should return a slightly left-panned but still "full amplitude"
    // signal. A value of 2 should give the same except on the right
    // side, and a 3 should give a centered but 1/4 amplitude signal.
    t_sample in;
...
{
    dsp_add(pannum_tilde_perform, 5, x,
            sp[0]->s_vec,
            sp[1]->s_vec,
            sp[2]->s_vec,
            sp[0]->s_n);
